...
import logo from './Logo2.JPG'; // Importa la imagen
...
function App() {
  ...
  const handleLogin = () => {
    ...
    if ((username === 'Gerzon' && password === '1201') || (username === savedUsername && password === savedPassword)) {
      setLoggedInUser(username === 'Gerzon' ? 'Gerzon' : savedUsername);
      setLoggedIn(true);
      ...
      {loggedIn && currentPage === 'main' && ( // Mostrar la página principal mientras currentPage sea 'main'
        <div>
          ...
          <div className="circle-container">
            {chartSize > 0 && (
              <div className="circle" style={{ background: `conic-gradient(${chartColor} ${chartSize}%, transparent ${chartSize}%)` }}></div>
            )}
          </div>
